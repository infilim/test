...
public class CameraTest : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        Vector3 temp = transform.position; // временна€ переменна€, чтобы Unity не жаловалс€
        temp.y = myHeight;
        transform.position = temp;

        // —оздаЄм точку выше центра автомобил€
        Vector3 lookAtPosition = car.position + new Vector3(0, height / 2, 0);

        // “еперь камера смотрит на новую точку
